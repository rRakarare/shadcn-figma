#!/usr/bin/env python3
"""
Fetch full node tree from Figma REST API.

Usage:
    python fetch_node_tree.py <file_key> <node_id> <output_path> [--format=json|xml]

Example:
    python fetch_node_tree.py abc123 2143:31510 ./output/node-tree.json
    python fetch_node_tree.py abc123 2143-31510 ./output/node-tree.xml --format=xml
...
def fetch_node_tree(file_key: str, node_id: str) -> Dict[str, Any]:
    ...
    # Normalize node_id (convert - to :)
    node_id = node_id.replace("-", ":")

    # Figma API endpoint for specific nodes (URL encode the node_id)
...
def node_to_xml(node: Dict[str, Any], indent: int = 0) -> str:
    """Convert a Figma node to XML format with proper indentation."""
    spaces = "  " * indent
    node_type = node.get("type", "UNKNOWN")
    node_id = node.get("id", "")
    node_name = node.get("name", "").replace("&", "&amp;").replace("<", "&lt;").replace(">", "&gt;").replace('"', "&quot;")
    ...
    # Get component info for instances
    component_id = node.get("componentId", "")

    # Build attributes
    attrs = f'id="{node_id}" name="{node_name}" type="{node_type}"'
    attrs += f' x="{x:.0f}" y="{y:.0f}" width="{width:.0f}" height="{height:.0f}"'
...
def extract_node_summary(node: Dict[str, Any], depth: int = 0) -> Dict[str, Any]:
    """Extract a summary of the node tree with essential info."""
    summary = {
        "id": node.get("id", ""),
        "name": node.get("name", ""),
        "type": node.get("type", ""),
    ...
    children = node.get("children", [])
    if children:
        summary["children"] = [extract_node_summary(child, depth + 1) for child in children]
...
def main():
    ...
    if output_format == "xml":
        ...
        # JSON format - save both full and summary
        summary = extract_node_summary(document)
        output = {
            "fileKey": file_key,
            "nodeId": node_id,
            "totalNodes": total,
        ...
        # Also save full raw response
        raw_path = output_path.replace(".json", "-raw.json")
        with open(raw_path, "w", encoding="utf-8") as f:
            json.dump(document, f, indent=2)
        print(f"\nFull raw data saved to: {raw_path}")
